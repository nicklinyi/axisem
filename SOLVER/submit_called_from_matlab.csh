...
foreach isim (${srcapp})
    ...
    if ( $newqueue == 'true' ) then
	 ...
	 else if ( $queue == 'irene') then

           echo '#\!/bin/bash -l' > axisem_solver.batch
     	   echo "#MSUB -r axisem_solver " >> axisem_solver.batch
           echo "#MSUB -q  skylake  " >> axisem_solver.batch
           echo "#MSUB -A  racoe003 " >> axisem_solver.batch
           echo "#MSUB -m  scratch,work,store ">> axisem_solver.batch
           echo "#MSUB -n " $4 >> axisem_solver.batch
           echo "#MSUB -N " $5 >> axisem_solver.batch
           echo "#MSUB -T 40000 " >> axisem_solver.batch
           echo "#MSUB -e  axisem_solver%I.e" >> axisem_solver.batch
           echo "#MSUB -o  axisem_solver%I.e" >> axisem_solver.batch
           echo "set -x" >> axisem_solver.batch
           echo ' cd \${BRIDGE_MSUB_PWD}' >> axisem_solver.batch
           echo " ccc_mprun ./axisem > OUTPUT" >> axisem_solver.batch
           ccc_msub axisem_solver.batch
