...
! Writes out a database file to be read by the solver for each processor.
  ...
  integer           :: lfdbname
  logical, parameter :: coupling=.true.
  real(kind=dp) ,parameter :: nb_digit_for_dt = 10000.
  real(kind=dp) :: dt_save
  ...
  do iproc=0, nproc-1
     ...
     if (coupling) then 
     !! VM VM add rules to have only 4 digit in dt for subsampling purpose and coupling efficency with specfem
        !write(*,*) " old time step ", iproc, dt_save
        dt = real( (int(dt_save*nb_digit_for_dt) - 1 ), dp ) / nb_digit_for_dt
        !write(*,*) " new time step ", iproc, dt
     end if
